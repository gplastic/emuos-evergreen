	...
	var game = {
			...
			if (sidebar.repairMode) {
				...
				if (selectedObject) {
					if (game.selectedUnits.length == 1 && selectedObject.selected && selectedObject.team == game.currentLevel.team) {
						...
						//My team's harvester is selected alone
						if (selectedObject.name == 'tiberium') {
							game.selectedUnits[0].orders = {
								type: 'harvest',
								to: {x: selectedObject.x, y: selectedObject.y}
							};
		...
		start: function () {
			...
			this.units.push(vehicles.add({
				...
				y: 6,
				team: 'nod',
				orders: {type: 'patrol', from: {x: 9, y: 24}, to: {x: 12, y: 8}}
			}));
			this.units.push(vehicles.add({
				name: 'light-tank',
				x: 2,
				y: 20,
				team: 'nod',
				orders: {type: 'patrol', from: {x: 2, y: 5}, to: {x: 6, y: 20}}
			}));
			this.units.push(vehicles.add({
				name: 'light-tank',
				x: 5,
				y: 10,
				team: 'nod',
				orders: {type: 'patrol', from: {x: 17, y: 12}, to: {x: 22, y: 2}}
			}));
			...
			this.units.push(vehicles.add({
				...
				y: 23,
				team: 'nod',
				orders: {type: 'patrol', from: {x: 4, y: 23}, to: {x: 22, y: 25}}
			}));
			this.units.push(vehicles.add({
				name: 'light-tank',
				x: 2,
				y: 10,
				team: 'nod',
				orders: {type: 'protect', target: game.units[0]}
			}));
			...
			"gdi1": {
				...
				scriptedEvents: [
					...
					},
					{
						...
						actions: [
							{
								action: 'wait', trigger: 'condition', condition: function () {
								return true;
							}
							},
