...
function score(s, type) {
	...
	SCORE += s;
	$('#score span').html(SCORE === 0 ? '00' : SCORE);
	...
	if (type && (type === 'clyde' || type === 'pinky' || type === 'inky' || type === 'blinky')) {
		...
		$('#board span.combo').css({
			top: (window['GHOST_' + type.toUpperCase() + '_POSITION_Y'] - 10) + 'px',
			left: (window['GHOST_' + type.toUpperCase() + '_POSITION_X'] - 10) + 'px'
		});

		SCORE_GHOST_COMBO = SCORE_GHOST_COMBO * 2;
	} else if (type && type === 'fruit') {
		$('#board').append('<span class="fruits">' + s + '</span>');
		$('#board span.fruits').css({
			top: (FRUITS_POSITION_Y - 14) + 'px',
			left: (FRUITS_POSITION_X - 14) + 'px'
		});
	}
}