...
function initGhost(ghost) {
	...
	window['GHOST_' + ghost.toUpperCase() + '_CANVAS_CONTEXT'] = canvas.getContext('2d');
}
...
function getGhostCanvasContext(ghost) {
	return window['GHOST_' + ghost.toUpperCase() + '_CANVAS_CONTEXT'];
}
...
function drawGhost(ghost) {
	...
	ctx.fillStyle = window['GHOST_' + ghost.toUpperCase() + '_STATE'] === 0 ? window['GHOST_' + ghost.toUpperCase() + '_COLOR'] : (window['GHOST_' + ghost.toUpperCase() + '_AFFRAID_STATE'] === 1 ? GHOST_AFFRAID_FINISH_COLOR : GHOST_AFFRAID_COLOR);
	drawHelperGhost(ctx, window['GHOST_' + ghost.toUpperCase() + '_POSITION_X'], window['GHOST_' + ghost.toUpperCase() + '_POSITION_Y'], window['GHOST_' + ghost.toUpperCase() + '_DIRECTION'], window['GHOST_' + ghost.toUpperCase() + '_BODY_STATE'], window['GHOST_' + ghost.toUpperCase() + '_STATE'], window['GHOST_' + ghost.toUpperCase() + '_AFFRAID_STATE']);
	ctx.closePath();
}
...
function affraidGhost(ghost) {
	...
	if (window['GHOST_' + ghost.toUpperCase() + '_STATE'] === 0 || window['GHOST_' + ghost.toUpperCase() + '_STATE'] === 1) {
		stopGhost(ghost);
...
function eatGhost(ghost) {
	...
	if (window['GHOST_' + ghost.toUpperCase() + '_STATE'] === 1) {
		...
		window['GHOST_' + ghost.toUpperCase() + '_STATE'] = -1;
		window['GHOST_' + ghost.toUpperCase() + '_EAT_TIMER'] = new Timer('cancelEatGhost("' + ghost + '")', GHOST_EAT_TIME);
...
function cancelEatGhost(ghost) {
	if (window['GHOST_' + ghost.toUpperCase() + '_STATE'] === -1) {
		window['GHOST_' + ghost.toUpperCase() + '_EAT_TIMER'] = null;
		stopGhost(ghost);
		window['GHOST_' + ghost.toUpperCase() + '_STATE'] = 0;
		moveGhost(ghost);
		testStateGhosts();
...
function moveGhost(ghost) {
	if (window['GHOST_' + ghost.toUpperCase() + '_MOVING'] === false) {
		window['GHOST_' + ghost.toUpperCase() + '_MOVING'] = true;
		var speed = window['GHOST_' + ghost.toUpperCase() + '_STATE'] === 1 ? GHOST_AFFRAID_MOVING_SPEED : (window['GHOST_' + ghost.toUpperCase() + '_STATE'] === 0 ? (window['GHOST_' + ghost.toUpperCase() + '_TUNNEL'] === false ? GHOST_MOVING_SPEED : GHOST_TUNNEL_MOVING_SPEED) : GHOST_EAT_MOVING_SPEED);
		...
		if (canMoveGhost(ghost)) {
			...
			if (window['GHOST_' + ghost.toUpperCase() + '_DIRECTION'] === 1) {
				window['GHOST_' + ghost.toUpperCase() + '_POSITION_X'] += GHOST_POSITION_STEP;
			} else if (window['GHOST_' + ghost.toUpperCase() + '_DIRECTION'] === 2) {
				window['GHOST_' + ghost.toUpperCase() + '_POSITION_Y'] += GHOST_POSITION_STEP;
			} else if (window['GHOST_' + ghost.toUpperCase() + '_DIRECTION'] === 3) {
				window['GHOST_' + ghost.toUpperCase() + '_POSITION_X'] -= GHOST_POSITION_STEP;
			} else if (window['GHOST_' + ghost.toUpperCase() + '_DIRECTION'] === 4) {
				window['GHOST_' + ghost.toUpperCase() + '_POSITION_Y'] -= GHOST_POSITION_STEP;
			}
...
function testGhostTunnel(ghost) {
	if (window['GHOST_' + ghost.toUpperCase() + '_STATE'] === 0) {
		if (isInTunnel(ghost) && window['GHOST_' + ghost.toUpperCase() + '_TUNNEL'] === false) {
			stopGhost(ghost);
			window['GHOST_' + ghost.toUpperCase() + '_TUNNEL'] = true;
			moveGhost(ghost);
		} else if (!isInTunnel(ghost) && window['GHOST_' + ghost.toUpperCase() + '_TUNNEL'] === true) {
			stopGhost(ghost);
...
function isInTunnel(ghost) {
	return ((window['GHOST_' + ghost.toUpperCase() + '_POSITION_X'] >= 2 && window['GHOST_' + ghost.toUpperCase() + '_POSITION_X'] <= 106) && window['GHOST_' + ghost.toUpperCase() + '_POSITION_Y'] === 258) || ((window['GHOST_' + ghost.toUpperCase() + '_POSITION_X'] >= 462 && window['GHOST_' + ghost.toUpperCase() + '_POSITION_X'] <= 548) && window['GHOST_' + ghost.toUpperCase() + '_POSITION_Y'] === 258);
}
...
function changeDirection(ghost) {
	...
	var ghostX = window['GHOST_' + ghost.toUpperCase() + '_POSITION_X'];
	var ghostY = window['GHOST_' + ghost.toUpperCase() + '_POSITION_Y'];
	...
	if (canMoveGhost(ghost, tryDirection) && (direction !== tryDirection - 2 && direction !== tryDirection + 2)) {
		window['GHOST_' + ghost.toUpperCase() + '_DIRECTION'] = tryDirection;
	}
}
...
function eraseGhost(ghost) {
	var ctx = getGhostCanvasContext(ghost);
	ctx.clearRect(window['GHOST_' + ghost.toUpperCase() + '_POSITION_X'] - 17, window['GHOST_' + ghost.toUpperCase() + '_POSITION_Y'] - 17, 34, 34);
}
...
function stopGhost(ghost) {
	...
	// noinspection DuplicatedCode
	if (window['GHOST_' + ghost.toUpperCase() + '_MOVING_TIMER'] !== -1) {
		clearInterval(window['GHOST_' + ghost.toUpperCase() + '_MOVING_TIMER']);
		window['GHOST_' + ghost.toUpperCase() + '_MOVING_TIMER'] = -1;
		window['GHOST_' + ghost.toUpperCase() + '_MOVING'] = false;
	}
}
