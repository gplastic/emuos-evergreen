                ...
                if (model) {
                    if (!this.codeActionsPromise) {
                        this.codeActionsPromise = async_1.createCancelablePromise(cancellationToken => {
                            return codeAction_1.getCodeActions(model, new range_1.Range(this.marker.range.startLineNumber, this.marker.range.startColumn, this.marker.range.endLineNumber, this.marker.range.endColumn), { type: 'manual', filter: { kind: types_2.CodeActionKind.QuickFix } }, cancellationToken).then(actions => {
