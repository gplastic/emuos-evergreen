(function (factory) {
	...
	var obj = {
		...
		find_match: function (pat_id, pattern, my_text) {
			...
			var re_match = my_text.match(obj.cached_patens[pat_id]);
			if (re_match) {
				re_match = re_match[0];
				var ret = obj.editor_svg.getModel().findMatches(re_match);
		...
		highlight_method_draw_selection: function() {
			...
			obj.timeout_highlight = setTimeout(function () {
				...
				if (obj.method_draw_selection) {
					...
					for (var el in obj.method_draw_selection.elements) {
						// noinspection JSUnfilteredForInLoop
						var pattern_2 = '(<\/?\\w*(?:(?:(?:\\s*(?:".*"|\'.*\'|[^\'"<\\s]?))?)+\\s*|\\s*)\\sid\\s*=\\s*["|\']?' + obj.method_draw_selection.elements[el].id + '["|\']?(?:(?:(?:\\s*(?:".*"|\'.*\'|[^\'">\\s]?))?)+\\s*|\\s*)\\w*\/?>)';
						// noinspection JSUnfilteredForInLoop
						var pattern_1 = '(<g\\w?(?:(?:(?:\\s*(?:".*"|\'.*\'|[^\'"<\\s]?))?)+\\s*|\\s*)\\sid\\s*=\\s*["|\']?' + obj.method_draw_selection.elements[el].id + '["|\']?(?:(?:(?:\\s*(?:".*"|\'.*\'|[^\'">\\s]?))?)+\\s*|\\s*)\\w*>)[\\S\\s]+?<\/g>';
						// noinspection JSUnfilteredForInLoop
						var el_string = methodDraw.canvas.svgToString(obj.method_draw_selection.elements[el]);
						...
						if (!match) {
							// noinspection JSUnfilteredForInLoop
							match = obj.find_match('p1_' + obj.method_draw_selection.elements[el].id, pattern_1, monaco_text);
						}

						if (!match) {
							// noinspection JSUnfilteredForInLoop
							match = obj.find_match('p2_' + obj.method_draw_selection.elements[el].id, pattern_2, monaco_text);
						}
